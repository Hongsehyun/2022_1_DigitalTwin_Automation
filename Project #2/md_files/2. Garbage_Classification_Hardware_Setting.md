...
Shield v2 의 핀아웃은 다음과 같습니다. 아두이노의 전압만으로도 스태퍼모터 구동이 가능하지만 스태퍼모터의 토크가 큰 경우 더 높은 전압이 필요할 수 있습니다. 그런 경우 Vin 에 외부 전압 (스태퍼모터 정격 전압의 3~5배) 를 인가합니다. 주의해야 할 점은 **외부 전압과 아두이노 전압을 동시에 인가하는 경우, Vin jumper 를 반드시 제거**해야 합니다. DC 모터 출력핀 M1 M2 , M3 M4 를 묶어서 사용하면 스태퍼모터를 구동시킬 수 있습니다. 순서는 **A+ A- B+ B-** 입니다.
