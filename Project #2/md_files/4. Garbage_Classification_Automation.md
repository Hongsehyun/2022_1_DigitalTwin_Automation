...
이후 분류된 쓰레기에 대한 정보를 텍스트 파일에 저장함으로 Robot_Operation.py로 정보를 넘겨줍니다. 
...
재활용품에 대한 정보를 읽어들이고난 후, 로봇은 재활용품을 분리수거하기 위해 움직인다. 로봇을 움직이기 위한 함수는 task_move_to이며, end-effector를 절대좌표로 이동시킬 수 있다. 절대좌표는 (x, y, z, roll, pitch, yaw)로 위치와 자세 정보를 담고 있다. 이번 프로그램에서는 위치 정보가 사전에 정의되어 있으며, 변수는 home_pos, grip_pos, mid_pos1, mid_pos2, mid_pos3 이다. mid_pos는 로봇의 움직임 반경 상 장애물을 피하도록 하기 위한 위치이다. 재활용품에 따른 위치 정보는 get_target_pos(garb_type)을 통해 얻을 수 있다. 함수 내부에 각 재활용품에 따른 위치가 저장되어 있다. 로봇이 분리수거를 완료하고나면, Home 위치로 돌아가며 WAIT 모드에 진입한다. 
...
'Garbage_Classification.py'와 'Robot_Operation.py' 두 파일을 동시에 실행함으로써 카메라와 로봇을 활용한 분리수거를 구현하였다. 두 프로그램은 독립적으로 실행되지만, 서로 간의 정보를 공유하며 각각의 프로세스를 실행하게 된다. 'garbage_type.txt'파일은 재활용품의 정보를 담고 있으며, 영상 처리와 학습된 모델을 통해 분류되어 저장된다. 'robot_state.txt'는 로봇의 상태 정보를 담고 있으며, 로봇이 움직이고 있는지 대기하고 있는 상황인지를 알려준다. 'Garbage_Classification.py'에서는 로봇이 현재 대기하고 있는 상황에서만 재활용품 분류작업을 수행한다. 로봇이 움직이는 상황에서는 분류작업을 실행하지 않는다. 'Robot_Operation.py'는 대기 상황에서 재활용품 정보를 읽는 것을 계속해서 시도한다. 만약 읽어진 정보가 있다면, 'garbage_type.txt'파일을 초기화시키고, 로봇이 움직이고 있다는 flag를 'robot_state.txt'에 저장한다. 그리고나서, 해당 재활용품을 분리수거를 하기 위해 움직인다. 분리수거가 완료되면, 로봇은 다시 대기 모드로 돌아가며, 대기 상태임을 flag를 저장함으로 알려준다. 
